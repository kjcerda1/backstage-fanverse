...
- **Notification routing / DM navigation / web video fix (2026-08-03, branch `claude/notification-dm-routing-web-video-010a20`, two rounds):** the notification popover previously always opened Backstage Buzz regardless of type — it now routes directly to a notification's destination in one tap, via a shared `resolveNotifDestination()` resolver used by both the popover and Buzz (no more diverging route tables). DM threads now scroll to the newest message reliably (previous scroll ran before attachment media finished loading, landing above the true bottom) and never re-scroll over a user's manual scroll-up. The DM header back arrow no longer loops/reopens the same thread when reached via a notification; back now exits directly to Fanverse (popover-origin) or Backstage Buzz (Buzz-origin) in one tap. **Browser Back was found to disagree with the app's own back arrow for Buzz-origin DMs (skipped straight to Fanverse instead of Buzz) — fixed so both controls now agree**; browser Forward is correctly a no-op in this app's reseal-on-pop history model. Desktop web browsers can now attach video (Windows Chrome/Edge often report an empty or generic MIME type for `.mov`/some `.mp4` files; a restricted extension fallback recovers those cases without weakening backend validation — confirmed backend `sanitizeDmMedia` does no content validation today, pre-existing and unchanged). Verified live with real QA-account DMs and real playable JPG/PNG/WebM/MOV/MP4 test files (not just synthetic Blobs), including repeated open/back cycles, reload persistence of read-state and video via signed URLs, and both themes at 375px/390px. Exact per-message scroll targeting (`targetMessageId`) is fully built and unit-tested client-side but dormant — the `notifications` table has no column for it; the exact migration + 3 backend edits needed are specified (not applied, needs approval) in the doc. All shipped changes are client-only — no SQL/schema/backend changes applied. Full root-cause detail, contracts, exact schema proposal, and the per-environment QA matrix: `docs/DM_NOTIFICATION_NAVIGATION_WEB_VIDEO_2026-08-03.md`.
