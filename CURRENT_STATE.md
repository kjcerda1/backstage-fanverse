...
- **Notification routing / DM navigation / web video fix (2026-08-03 through 2026-08-04, branch `claude/notification-dm-routing-web-video-010a20`, three rounds):** the notification popover previously always opened Backstage Buzz regardless of type — it now routes directly to a notification's destination in one tap, via a shared `resolveNotifDestination()` resolver used by both the popover and Buzz. DM threads scroll to the newest message reliably, and now scroll to and briefly highlight the **exact target message** when a notification carries one — real, not a placeholder: `notifications.target_message_id` (uuid, `REFERENCES messages(id) ON DELETE SET NULL`, migration `supabase-notifications-target-message-migration.sql`) is populated on every DM send with that message's real id (never inferred from the thread id) and flows through the API response, the resolver, and one-time scroll/highlight state; a deleted or unavailable target safely falls back to the newest message via the FK's `SET NULL`, verified live by deleting a real target message mid-QA. The DM header back arrow no longer loops/reopens the same thread when reached via a notification; back exits directly to Fanverse (popover-origin) or Backstage Buzz (Buzz-origin) in one tap — both the in-app arrow **and** the OS/browser Back button now agree (a real disagreement between the two was found and fixed for the Buzz-origin case). Desktop web browsers can now attach video (a restricted extension fallback recovers browsers that report an empty/generic MIME for `.mov`/`.mp4`), and — closing the trust gap that fallback opened — the backend now runs real, server-authoritative content validation (`detectMediaSignature`/`verifyDmMediaContent`: magic-byte/container-signature detection against the actual downloaded bytes, not the client's claim) before a DM photo/video ever becomes a real message; a rejected file (verified live: a plain-text file renamed `.mp4`) is deleted from storage and never becomes a sendable attachment. Full root-cause detail, the exact schema/migration rationale, the media-validation contract, and the per-environment QA matrix (including Desktop Edge/installed-PWA marked for owner verification with exact manual steps, not silently assumed): `docs/DM_NOTIFICATION_NAVIGATION_WEB_VIDEO_2026-08-03.md`.
- **Owner-device follow-up (2026-08-05, same branch, round 4, code-complete + automated-QA-passed — NOT yet owner-verified on device):** real installed-iPhone-PWA QA found rounds 1–3's browser-only pass had missed 5 defects. Root causes traced and fixed: (1) push notifications carried a top-level FCM `notification` block, which makes Firebase's Web SDK auto-display *and* auto-register its own competing `notificationclick` handler (driven by the bare app-root `fcmOptions.link`) — that's what was actually opening the general app instead of the exact DM; every push is now data-only, so this app's own deep-link-aware handler is the sole handler. (2) The DM inbox was never sorted by real activity (no raw timestamp existed to sort by — only a formatted display string); added a real `lastMessageAt` per thread + `sortConvosByActivity()` applied at render, plus a 15s poll so an incoming message can reorder the inbox without reopening the whole DM modal. (3) Voice-note sends failed after ~4s because `recorder.mimeType` on iOS Safari can report a codecs-qualified string (e.g. `"audio/mp4;codecs=mp4a.40.2"`) that fails the `dm-media` bucket's exact-match MIME allowlist at the storage-upload step — the bucket already allowlists bare `audio/mp4`/`audio/webm`/etc. (verified live), the fix strips the codecs parameter once at the source. (4) Added a real live waveform (Web Audio `AnalyserNode` reading the actual mic stream, 20 bars, `requestAnimationFrame`-driven, with a clearly-non-live animated fallback if the analyser can't attach) with full cleanup (stream/analyser/`AudioContext`/rAF) on cancel, finish, error, and unmount. (5) All three composer states (normal/recording/preview) now respect `env(safe-area-inset-bottom)` (previously flat padding with no inset at all) and use 44×44 primary tap targets. Also hardened the notification one-shot-target consumption against a StrictMode double-invoke data-loss pattern (dev-only; never affected production, fixed anyway per the task). 166/166 automated tests pass (`node tests/dm-owner-device-followup.test.js` new this round) and `npm run build` is clean, but **none of the 5 fixes have been re-verified on the owner's real device yet** — no physical iPhone/installed-PWA/microphone hardware in this environment. Full root-cause detail and the exact owner-verification checklist: `docs/DM_OWNER_DEVICE_FOLLOWUP_2026-08-05.md`.
