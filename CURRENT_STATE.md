...
- **Notification routing / DM navigation / web video fix (2026-08-03, branch `claude/notification-dm-routing-web-video-010a20`):** the notification popover previously always opened Backstage Buzz regardless of type — it now routes directly to a notification's destination in one tap, via a shared `resolveNotifDestination()` resolver used by both the popover and Buzz (no more diverging route tables). DM threads now scroll to the newest message reliably (previous scroll ran before attachment media finished loading, landing above the true bottom) and never re-scroll over a user's manual scroll-up. The DM header back arrow no longer loops/reopens the same thread when reached via a notification (a stale one-shot target was re-triggering the auto-open effect); back now exits directly to Fanverse (popover-origin) or Backstage Buzz (Buzz-origin) in one tap, verified live with real QA-account DMs including repeated open/back cycles. Desktop web browsers can now attach video (previously photos worked but videos silently failed — Windows Chrome/Edge often report an empty or generic MIME type for `.mov`/some `.mp4` files; a restricted extension fallback now recovers those cases without weakening backend validation). All client-only — no SQL/schema/backend changes. Full root-cause detail, contracts, and QA matrix: `docs/DM_NOTIFICATION_NAVIGATION_WEB_VIDEO_2026-08-03.md`.
